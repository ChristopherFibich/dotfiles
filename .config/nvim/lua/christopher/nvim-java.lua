        ...
        actions.select_default:replace(function()
          ...
          vim.ui.input({
            ...
            if new_path and new_path ~= '' then
              local new_content = file_content:gsub('(<classpathentry[^>]-path=")' .. selection.value.path .. '(")', '%1' .. new_path .. '%2')
        ...
        actions.select_default:replace(function()
          ...
          vim.ui.input({
            ...
            if new_location and new_location ~= '' then
              local new_content = file_content:gsub('(<location>).-</location>', '%1' .. new_location .. '</location>')
